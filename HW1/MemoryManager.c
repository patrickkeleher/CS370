...
int other =0;
int itercount= get_iteration_count(rand()); // Iteration count
int i=0;
...
while(i<itercount){
 int alength= get_arr_size(rand());
int *holder= (int*) malloc(alength * sizeof(int));
int j=0;
while(j<alength){
  holder[j]= rand();
  j++;
...
i++;
int k= return_median(holder, alength);
if((k % 13) == 0){
  other++;
}
  free(holder);
